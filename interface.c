...
CRB_create_interpreter(void)
{
    ...
    crb_add_native_functions(interpreter);  /* 为解释器装载函数内部方法，如: print() */
...
{
    extern int yyparse(void);  /* 运行yyparse()方法，开始lex与yacc的词法分析与语法分析，最终利用程序源文件构建分析树 */
    int error_code;
...
CRB_interpret(CRB_Interpreter *interpreter)
{
    ...
    if ((error_code = setjmp(interpreter
        ...
        result = crb_execute_statement_list(interpreter, NULL,
                                            interpreter->statement_list);  /* 执行程序语句 */
